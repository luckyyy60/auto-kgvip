...
先确认键名。正式键名是 `kg:accounts`（**冒号**），早期版本和「按导出文件名导入」的场景容易写成
`kg_accounts`（下划线），两者不通用，只认一种就会表现为「数据导入后不生效」。检查命令：

```bash
npx wrangler kv key list --namespace-id <你的 KV id> --remote
```

若列出的是下划线键名，当前代码会自动回退读取并迁移到正式键名，重新部署一次即可；
也可以手动把值复制到 `kg:accounts` / `kg:device` / `kg:settings` / `kg:logs`。

**Q：免费版 Worker 够用吗？**
够用。Cron 触发器免费版可用；KV 免费额度是**读 10 万/天、写 / 删 / 列出各 1000/天**。
...
按 5 个账号估算，日常写操作约 **40~60 次/天**，远低于 1000 的上限；代码里也没有任何 `list` 调用。
另外注意：**Dashboard 和 wrangler 的操作同样计入额度**，KV 页面每次刷新/搜索都算 list 请求。
